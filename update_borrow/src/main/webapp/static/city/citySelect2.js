 ...
 */
 ...
 CitySelect.prototype = {
	...
	_provFunc: function(){
		...
		/*--------------------------------------------------------------*/
		var strr = _config.provId + ">.select2-choice>.select2-chosen";
		var $regProv = $("#s2id_"+ strr.replace('#',''));
		$regProv.html("请选择");
		/*--------------------------------------------------------------*/
		
		var elemHtml = '';
		$(_config.provId).append('<option value="-1" data-name="请选择">请选择</option>');

		$(_config.data).each(function(i,curItem){
			$(_config.provId).append("<option data-name='"+curItem[1]+"' value='"+curItem[0]+"'>"+curItem[1]+"</option>");
		});
		
		if(_config.isSelect) {
			$($(_config.provId + " option")[1]).prop("selected",true);
			if(this.provCode) {
	...
	_cityFunc: function(){
		...
		/*----------------------------------------------------------------*/
		var strr = _config.cityId + ">.select2-choice>.select2-chosen";
		var $regCity = $("#s2id_"+ strr.replace('#',''));
		$regCity.html("请选择");
		/*----------------------------------------------------------------*/
		...
		var opthtml = '<option value="-1" data-name="请选择" >请选择</option>';
		...
		citys = prov[2];
		$(_config.cityId).append('<option value="-1" data-name="请选择">请选择</option>');
		for(var m = 0; m < citys.length; m++) {
			var oneCity = citys[m];
			$(_config.cityId).append("<option value='"+oneCity[0]+"' data-name='"+oneCity[1]+"'>"+oneCity[1]+"</option>");
		}
		//$(_config.cityId).html(elemHtml);
		
		if(_config.isSelect) {
			$($(_config.cityId + " option")[1]).prop("selected", true);
			if(this.cityCode) {
	...
	_areaFunc: function(){
		...
		/*-----------------------------------------------------------------*/
		var strr = _config.areaId + ">.select2-choice>.select2-chosen";
		var $regDistinct = $("#s2id_"+ strr.replace('#',''));
		...
		var provIndex = $(_config.provId).get(0).selectedIndex,
			...
			areas,
			oneCity;
		//$(_config.areaId).append('<option value="-1" data-name="请选择" selected>请选择</option>');
		
		/*
		 * 如果省选择了 "请选择" 那么省和市自动切换到请选择 那么相应的索引index-1
		...
		if(provIndex < 0 || cityIndex < 0){
			$(_config.cityId).attr("data-code",'-1');
			$(_config.areaId).attr("data-code",'-1');
			$(_config.areaId).html('<option value="-1" data-name="请选择" >请选择</option>');
			return;
		};
		prov = _config.data[provIndex];
		citys = prov[2];
		...
		if(areas != null) {
			for(var n = 0; n < areas.length; n++) {
				var oneArea = areas[n];
				$(_config.areaId).append("<option value='"+oneArea[0]+"' data-name='"+oneArea[1]+"'>"+oneArea[1]+"</option>");
			}
		...
		if(_config.isSelect) {
			$($(_config.areaId + " option")[1]).prop("selected", true);
			if(this.areaCode) {
    ...
    _changeSetCode: function(selectId){
		...
		var html = "<option data-name='请选择' value='-1'>请选择</option>",
			provOpt = $($(_config.provId + ' option')[0]).attr("value");
