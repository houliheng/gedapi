...
//        }
        ...
        function formatNode(tr) {
        	 ...
        	 * @date-author:20150706-zhunan: 根据树表中tr选项的hasChild值判断是否有孩子节点，当为true是否表示是，反之为否
        	 */
            if ($cur.attr('hasChild')=="true" || ($cur.attr('hasChild')&&$cur.attr('hasChild')!="false")) {
    	 ...
    	 * @date-designer:20150706-zhunan
    	 * @e-out-other : treeTableList - 树表 : 展示区域树的grid
    	 * @date-author:20150706-zhunan: 展开指定tr的孩子节点内容
    	 */
        $treeTable.openNode = function(tr){
        	if(tr.length == 0){
        		return;
        	}
        	tr.find('.' + css['AN']).attr('class', css['AN'] + ' ' + css['LO']);
        	opts.beforeExpand($treeTable, tr.attr('id'));
            //展开所有直属节点，根据图标展开子孙节点
            open(tr.attr('id'));
        }
