...
<head>
	...
	<script type="text/javascript">
		...
		function uploadFile(id){
			var url = "${ctx}/credit/sys/template/uploadView?fileType=2&id="+id;
	 ...
	 * @date-author:20151020-huangxuecheng:系统管理-资料核查模板配置信息页面，修改新增js方法，全部使用ajax提交然后，返回一个值判断是新增或者修改或者删除
	 * 原生的js校验
	 */
		//新增
		function addView(productType){
			var url = "${ctx}/credit/sys/template/addDoubtfulView?fileType=2&productType="+productType;
			var returnVal = window.showModalDialog(url,null,"dialogWidth:500px;dialogHeight:250px;status:no;help:no;resizable:yes;");
			if(productType!=''){
				if(returnVal==1){
					returnVal = 2;
				}else{
			...
			var finalUrl = "${ctx}/credit/sys/template/templateList?fileType=2" + "&success_flag=" + returnVal;
		...
		function deleteData(){
			...
			if(str!=""){
				var url = "${ctx}/credit/sys/template/deleteAll?fileType=2&idStr="+str;
...
<body>
	<!-- 
	 ...
	 @reqno:AH1509130053
	 @date-designer:20151020-huangxuecheng
	 @date-author:20151020-huangxuecheng:系统管理-资料核查模板配置信息页面，修改以前留下的问题，增加productType隐藏回显，改变以前的submit按钮改成button后使用js的ajax提交而不是form提交
	...
	-->
 	<div class="wrapper">
		<div class="searchInfo">
	        ...
	        <div class="searchCon">
	        	<form:form id="searchForm" modelAttribute="template"  action="${ctx}/credit/sys/template/templateList?fileType=2" method="post" >
					...
					<div class="filter">
						<table class="searchTable">
							<tr>
								...
								<td class="ft_content">
									<form:input id="templateName" path="templateName" htmlEscape="false" maxlength="50" class="input-medium"/>
								</td>
		...
		<!--
	    @reqno:H1601160030
	    @date-designer:2016年1月18日-lihao02
	    @date-author:2016年1月18日-lihao02:weblogic 下资料核查模板管理下上传txt或者xlsx格式文件后无法下载
	 	如果文件不存在，提示信息修改为error；
	    -->
		    	...
		    	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		        	...
		        	<c:forEach items="${page.list}" var="template"  varStatus="index">
						...
						<c:if test="${1 == index.count%2}"><tr></c:if> 
							...
							<td width="150px;"><c:out value="${template.productTypeName}"></c:out></td>
							<td><c:out value="${template.templateName}"></c:out></td>
							<td width="160px;"><fmt:formatDate value="${template.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
							<td width="150px;">
								...
								<a href="${ctx}/credit/sys/template/download?id=${template.id}&fileType=2" onclick="locaurl(this.href)">模板下载</a>
			 				</td>
						</tr>
					</c:forEach>
		        </table>
