...
<meta name="decorator" content="default" />
	<!--  
  	@reqno:H1510230035
  	@date-designer:20151104-huangxuecheng
  	@date-author:20151104-huangxuecheng:CRE_信贷审批_任务中心_待办理任务列表_查询需求概述：查看当前用户的待办理任务列表，这个待办理任务列表是根据传入的taskDefId如果包含了utask_batch便进行渲染跳转的页面，使其功能区别于普通待办任务
...
/* 		var str = getCheckBoxVal();
		if(str != ""){	
			if(str.indexOf(",") != str.length-1){
		    	...
		    	var url = "${ctx}/credit/taskCenter/doBusiness/" + procDefId + "/" + execId + "/" + taskDefKey;
		    	$.ajax({
					type:"post",
					url:url,
					dataType:"json",
					success:function(data){
						//url
				 		...
				 		//flag表示未办理0已办理1
				 		var flag = "0"; 
				 		var newUrl = "${ctx}" + location + "?procDefId=" + procDefId + "&taskDefKey=" + taskDefKey + "&applyId=" + id + "&execId=" + execId + "&flag=" + flag;
				 		var windowWidth = window.parent.window.document.body.offsetWidth -50;
				 		var windowHeight = window.parent.window.document.body.offsetHeight - 50;
				 		window.showModalDialog(newUrl, null, "dialogWidth:" + windowWidth + "px" + ";dialogHeight:" + windowHeight + "px" + ";status:no;help:no;resizable:yes;");
					},
					error:function(msg){
						alert("未能办理，请查看后台信息");
					}
	...
	function doSignBusiness(procDefId, execId, taskDefKey, id, title,taskId,procInstId){
		...
		$.ajax({
			...
			success:function(data){
				...
				//flag表示未办理0已办理1
			    var flag = "0"; 
				var newUrl = "${ctx}" + location + "?procDefId=" + procDefId + "&taskDefKey=" + taskDefKey + "&applyId=" + id + "&execId=" + execId + "&flag=" + flag + "&title=" + title + "&taskId=" + taskId+"&procInstId="+procInstId+"&canRedo="+canRedo;
				var windowWidth = window.parent.window.document.body.offsetWidth - 50;
				var windowHeight = window.parent.window.document.body.offsetHeight - 50;
				window.showModalDialog(newUrl, null, "dialogWidth:" + windowWidth + "px" + ";dialogHeight:" + windowHeight + "px" + ";status:no;help:no;resizable:yes;");
			},
	...
	function doBatchSubmit(){
		...
		if(str!=""){
			...
			var finalUrl = "${ctx}/credit/taskCenter/list/${taskDefKey}?returnVal=" + returnVal;
	...
	function doExport(){
		...
		if(str!=""){
			...
			//status0是待办1是已办
			var url = "${ctx}/credit/taskCenter/list/export/${taskDefKey}?procInsIds=" + str + "&taskIds=" +id + "&taskDefKey=" +taskDefKey + "&status=1";
...
<body>
	<div class="wrapper">
		<div class="searchInfo">
			...
			<div class="searchCon">
				<form:form id="searchForm" modelAttribute="customer" action="${ctx}/credit/taskCenter/toDoneList/batch/${taskDefKey}" method="post">
		...
		<div class="ribbon">
			<ul class="layout">
				<li class="mcp_info"><a href="#" id="processDo" onclick="doBusiness()"><span><b></b>详情</span></a></li>
		...
		<div class="tableList">
			...
			<table cellpadding="0" cellspacing="0" border="0" width="100%" style="table-layout:auto;">
				<tr>
					...
					<th ><div >放款户名</div></th>
					<th ><div >放款银行</div></th>
					<th ><div >合同金额(元)</div></th>
					<th ><div >合同编号</div></th>
					<th ><div >合同签订日期</div></th>
					<th ><div >申请金额(元)</div></th>
					<th ><div >登记门店</div></th>
				</tr>
					<c:forEach items="${page.list}" var="process" varStatus="index">
						...
						<td ><div class="title"><a id="c_${index.count}_alink" href="javascript:void(0)" onclick="doSignBusiness('${process.PROCESSDEFINITIONID }','${process.EXECUTIONID }','${process.TASKDEFINITIONKEY }','${process.APPLY_ID }','${process.NAME}','${process.ID }','${process.PROC_INS_ID}')">${process.NAME}</a></div></td>
						...
						<td ><div class="title"><fmt:formatDate value="${process.ENDTIME }" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></div></td>
