...
<script type="text/javascript">
	...
	function qy_s5submit() {
		var passIdent = false, remarkIdent = false;
		<c:forEach items="${qy_fcdyjkshList}" var ="ruleFom" varStatus="i">
		if (checkIsNull($("[name=qy_s5passFlag${i.index}]:checked").val())) {
			passIdent = true;
		}
		if ($("[name=qy_s5passFlag${i.index}]:checked").val() == "0") {
			if (checkIsNull($("#qy_s5remark${i.index}").val())) {
				remarkIdent = true;
			}
		}
		</c:forEach>
		if (remarkIdent) {
			...
			var formJson = "[";
			<c:forEach items="${qy_fcdyjkshList}" var ="ruleFom" varStatus="i">
			formJson += "{'reviewedBook':'${ruleFom.id}','passFlag':" + $("input[name='qy_s5passFlag${i.index}']:checked").val() + ",'remark':'" + $("#qy_s5remark${i.index}").val() + "'},";
			</c:forEach>
	...
	function qy_fcdyjkshClick() {
		$("#qy_fcdyjksh").toggle(600);
	}
</script>
<div class="wrapper">
	<div class="tab-content">
		<div class="tableList">
			<h3 onclick="qy_fcdyjkshClick()" class="tableTitle">房产抵押借款审核</h3>
			<div id="qy_fcdyjksh">
						...
						<c:forEach items="${fg_fcdyjkshList}" var="rule" varStatus="i">
							...
							<tr>
								<td rowspan="1">
									<label>区域审核 :</label>
									&nbsp;&nbsp;
									<input type="radio" name="qy_s5passFlag${i.index}" value="0" id="qy_s5passFlag${i.index}" class="required">
									<label for="radio_yes">碰触</label>
									&nbsp;&nbsp;
									<input type="radio" name="qy_s5passFlag${i.index}" value="1" id="qy_s5passFlag${i.index}1" class="required">
									<label for="radio_no">不碰触</label>
									<font style="color: red">*</font>
								</td>
								<td>
									<textarea id="qy_s5remark${i.index}" maxlength="1000" style="height: 70px;"></textarea>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<div id="fq_s5saveButton" class="form-actions">
					<shiro:hasPermission name="credit:reviewedRule:edit">
						<input id="btnSubmit" class="btn btn-primary" onclick="qy_s5submit()" type="button" value="保 存" />&nbsp;</shiro:hasPermission>
				</div>
			</div>
		</div>
	</div>
