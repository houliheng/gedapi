...
<script type="text/javascript">
	...
	function onCheck(filePath, pageNo) {
		...
		$.ajax({
					...
					success : function(data) {
						...
						var feilServerUrl = returnObj[length].feilServerUrl;
						/* <img height='140' data-original='"+feilServerUrl
						...
						for (var i = 0; i < returnObj.length - 1; i++) {
							...
							if(((returnObj[i].thumbnailStoragePath)+'').indexOf('data:image/jpeg;base64')=='-1'){
								if(checkIsNull((returnObj[i].thumbnailStoragePath)+'')){
									...
									Str += "<li><img height='140' data-original='"+feilServerUrl
											+ returnObj[i].fileStoragePath
											+ "' src='"+feilServerUrl
											+ returnObj[i].fileStoragePath
											+ "' alt='"
	...
	$.ajax({
		type : "post",
		url : "${ctx}/credit/ApplyRegisterVO/showTree?ids=" + applyNo
				+ "&taskDefKey=" + taskDefKey,
		dataType : "json",
		success : function(data) {
			//请求成功后处理函数
