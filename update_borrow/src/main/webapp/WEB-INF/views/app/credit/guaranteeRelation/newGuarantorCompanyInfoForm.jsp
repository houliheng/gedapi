...
<script type="text/javascript">
	...
	function saveCompanyInfo() {
		if(($("#companyRegDistinct").val() == "-1") || ($("#operateDistinct").val() == "-1")){
			...
			loading();
			var guaranteeCompanyId = '${guaranteeCompanyId}';
			...
			var applyNo = $("#currApplyNo").val();
			$.post("${ctx}/credit/creApplyCompanyRelation/saveGuarantee?flag=1&&guaranteeCompanyId="+guaranteeCompanyId, company, function(data) {
				if (data) {
					closeTip();
					if (data.status == 1) {
						alertx(data.message, function() {
							parent.$.loadDiv("guarantorCompanyInfo", "${ctx}/credit/guarantorCompanyInfo/guaranteeList", {
				                custId : guaranteeCompanyId,
				                applyNo : applyNo
				            }, "post");
							closeJBox();
						});
	...
	<div class="searchInfo">
		...
		<div class="searchCon">
			<form:form id="guarantorCompanyInfoForm" modelAttribute="guarantorCompanyInfo" action="" method="post" class="form-horizontal">
				<form:hidden path="id" />
				<input type="hidden" id="currApplyNo" name="currApplyNo" value="${guarantorCompanyInfo.currApplyNo }" />
