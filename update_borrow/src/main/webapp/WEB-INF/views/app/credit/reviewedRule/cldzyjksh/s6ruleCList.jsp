...
<script type="text/javascript">
	$(function() {
		$("#dq_s6saveButton").hide();
		$("textarea").css("width", $(window).width() * 0.22 - 30);
		if ("${checkReviewedId}" == "true" && "${readOnly}" == "false") {
			//document.getElementById("dq_s6saveButton").style.visibility = "visible";
			$("#dq_s6saveButton").show();
		...
		<c:forEach items="${dq_fcdzyjkshList}" var ="ruleFom" varStatus="i">
		var value = "${ruleFom.passFlag}";
		$("input[name= dq_s6passFlag${i.index}][value=" + value + "]").attr("checked", true);
		$("#dq_s6remark${i.index}").val('${ruleFom.remark}');
		</c:forEach>
	});

	function dq_s6submit() {
		var passIdent = false, remarkIdent = false;
		<c:forEach items="${dq_fcdzyjkshList}" var ="ruleFom" varStatus="i">
		if (checkIsNull($("[name=dq_s6passFlag${i.index}]:checked").val())) {
			passIdent = true;
		}
		if ($("[name=dq_s6passFlag${i.index}]:checked").val() == "0") {
			if (checkIsNull($("#dq_s6remark${i.index}").val())) {
				remarkIdent = true;
			}
		}
		</c:forEach>
		if (remarkIdent) {
			...
			var formJson = "[";
			<c:forEach items="${dq_fcdzyjkshList}" var ="ruleFom" varStatus="i">
			formJson += "{'reviewedBook':'${ruleFom.id}','passFlag':" + $("input[name='dq_s6passFlag${i.index}']:checked").val() + ",'remark':'" + $("#dq_s6remark${i.index}").val() + "'},";
			</c:forEach>
						...
						<c:forEach items="${fg_fcdzyjkshList}" var="rule" varStatus="i">
							...
							<tr>
								<td rowspan="1">
									<label>区域审核 :</label>
									&nbsp;&nbsp;
									<input type="radio" name="qy_s6passFlag${i.index}" disabled="disabled" value="0" id="qy_s6passFlag${i.index}" class="required">
									<label for="radio_yes">碰触</label>
									&nbsp;&nbsp;
									<input type="radio" name="qy_s6passFlag${i.index}" disabled="disabled" value="1" id="qy_s6passFlag${i.index}1" class="required">
									<label for="radio_no">不碰触</label>
									<font style="color: red">*</font>
								</td>
								<td>
									<textarea id="qy_s6remark${i.index}" readonly="readonly" maxlength="1000" style="height: 70px;"></textarea>
								</td>
							</tr>
							<tr>
								<td rowspan="1">
									<label>大区审核 :</label>
									&nbsp;&nbsp;
									<input type="radio" name="dq_s6passFlag${i.index}" value="0" id="dq_s6passFlag${i.index}" class="required">
									<label for="radio_yes">碰触</label>
									&nbsp;&nbsp;
									<input type="radio" name="dq_s6passFlag${i.index}" value="1" id="dq_s6passFlag${i.index}1" class="required">
									<label for="radio_no">不碰触</label>
									<font style="color: red">*</font>
								</td>
								<td>
									<textarea id="dq_s6remark${i.index}" maxlength="1000" style="height: 70px;"></textarea>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<div id="dq_s6saveButton" class="form-actions">
					<shiro:hasPermission name="credit:reviewedRule:edit">
						<input id="btnSubmit" class="btn btn-primary" onclick="dq_s6submit()" type="button" value="保 存" />&nbsp;</shiro:hasPermission>
				</div>
			</div>
		</div>
	</div>
