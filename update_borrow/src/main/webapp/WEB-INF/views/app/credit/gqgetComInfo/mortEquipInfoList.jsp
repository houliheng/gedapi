...
<script type="text/javascript">
	...
	function addEquip(){
		openJBox('addEquipBox',"${ctx}/credit/gqgetComInfo/equipform","新增抵押设备信息",700,350,{applyNo:$("#applyNo").val()});
	...
	function editEquip(){
    	...
    	if(id.length != 1){
			...
			openJBox('editEquiprBox',"${ctx}/credit/gqgetComInfo/equipform","编辑抵押设备信息",700,350,{id:id,applyNo:$("#applyNo").val()});
    	}
	}
	function deleteEquip(){
    	...
    	if(0 == ids.length){
    		...
    		confirmx("是否删除该设备?",function(){
    			$.post("${ctx}/credit/gqgetComInfo/equipdelete?id="+ids,null,function(data){
    				console.log(data);
   		    		if(data.status == '1'){
   		    			alertx(data.message,function(){
	   		    			$.loadDiv('equip', '${ctx}/credit/gqgetComInfo/mortEquip', {applyNo:$("#applyNo").val()}, 'post');
   		    			});
   		    		}else{
   		    			alertx(data.message);
   		    		}
					...
					<table id="fromTable filter" class="table table-striped table-bordered table-condensed">
						...
						<tbody>
							<c:forEach items="${mortEquipInfoList}" var="mortEquip" varStatus="i">
								<tr>
									<td width="20px">
										<input type="checkbox" value="${mortEquip.id}" name="equipCheck">
									</td>
