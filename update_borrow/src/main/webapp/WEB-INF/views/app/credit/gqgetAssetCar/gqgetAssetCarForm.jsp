...
<script type="text/javascript">
	...
	function saveForm() {
		...
		$.post("${ctx}/credit/gqgetAssetCar/save", formJson, function(data) {
			if (data) {
				if (data.status == 1) {//保存成功
					alertx(data.message, function() {
						parent.$.loadDiv("assetCar", "${ctx }/credit/gqgetAssetCar/list", {
							gqgetApplyNo : data.gqgetApplyNo
						}, "post");
						closeJBox();
					});
				} else {
...
<body>
	<form:form id="gqgetAssetCarForm" modelAttribute="gqgetAssetCar" action="${ctx}/credit/gqgetAssetCar/save" method="post" class="form-horizontal">
		<form:hidden path="id" />
