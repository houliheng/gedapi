...
<script type="text/javascript">
	...
	function qy_s4_2submit() {
		var passIdent = false, remarkIdent = false;
		<c:forEach items="${qy_2hhjkshList}" var ="ruleFom" varStatus="i">
		if (checkIsNull($("[name=qy_s4_2passFlag${i.index}]:checked").val())) {
			passIdent = true;
		}
		if ($("[name=qy_s4_2passFlag${i.index}]:checked").val() == "0") {
			if (checkIsNull($("#qy_s4_2remark${i.index}").val())) {
				remarkIdent = true;
			}
		}
		</c:forEach>
		if (remarkIdent) {
			...
			var formJson = "[";
			<c:forEach items="${qy_2hhjkshList}" var ="ruleFom" varStatus="i">
			formJson += "{'reviewedBook':'${ruleFom.id}','passFlag':" + $("input[name='qy_s4_2passFlag${i.index}']:checked").val() + ",'remark':'" + $("#qy_s4_2remark${i.index}").val() + "'},";
			</c:forEach>
	...
	function qy_2hhjkshClick() {
		$("#qy_2hhjksh").toggle(600);
	}
</script>
<div class="wrapper">
	<div class="tab-content">
		<div class="tableList">
			<h3 onclick="qy_2hhjkshClick()" class="tableTitle">混合贷借款额在抵质押物市值的100%到150%之间的</h3>
			<div id="qy_2hhjksh">
						...
						<c:forEach items="${fg_2hhjkshList}" var="rule" varStatus="i">
							...
							<tr>
								<td rowspan="1">
									<label>区域审核 :</label>
									&nbsp;&nbsp;
									<input type="radio" name="qy_s4_2passFlag${i.index}" value="0" id="qy_s4_2passFlag${i.index}" class="required">
									<label for="radio_yes">碰触</label>
									&nbsp;&nbsp;
									<input type="radio" name="qy_s4_2passFlag${i.index}" value="1" id="qy_s4_2passFlag${i.index}1" class="required">
									<label for="radio_no">不碰触</label>
									<font style="color: red">*</font>
								</td>
								<td>
									<textarea id="qy_s4_2remark${i.index}" maxlength="1000" style="height: 70px;"></textarea>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<div id="fq_s4_2saveButton" class="form-actions">
					<shiro:hasPermission name="credit:reviewedRule:edit">
						<input id="btnSubmit" class="btn btn-primary" onclick="qy_s4_2submit()" type="button" value="保 存" />&nbsp;</shiro:hasPermission>
				</div>
			</div>
		</div>
	</div>
