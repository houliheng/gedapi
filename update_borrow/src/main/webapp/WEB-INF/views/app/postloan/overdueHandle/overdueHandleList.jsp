...
<head>
	...
	<script type="text/javascript">
		...
		//结论
		function handle(procDefId, execId, taskDefKey, applyNo, title,taskId,procInstId,contractNo,periodNum){
			contractNo = encodeX(contractNo);
	 		var url = "${ctx}/postloan/overdueHandle/doBusiness?procDefId=" + procDefId + "&execId=" + execId + "&taskDefKey=" + taskDefKey + "&taskId=" + taskId + "&procInstId=" + procInstId + "&contractNo=" + contractNo + "&periodNum=" + periodNum + "&applyNo=" + applyNo;
	 		var height = $(window).height() - 50;
			var width = $(window).width() - 100;
			//var url = "${ctx}/postloan/checkTwentyFive/toDetailsPage?" +"applyNo=" + applyNo;
...
<body>
	<div class="wrapper">
		<div class="searchInfo">
			...
			<div class="searchCon">
				<form:form id="searchForm" modelAttribute="overdueHandle" action="${ctx}/postloan/overdueHandle/list/${taskDefKey}" method="post" >
					...
					<c:forEach items="${page.list}" var="process" varStatus="index">
						...
						<c:if test="${1 == index.count%2}">
							...
							<c:if test="${process.TASKDEFINITIONKEY eq 'utask_dqxf'}">
								<td><a href="javascript:void(0)" onclick="dqxfAssignList('${process.ID}','${process.PROCESSINSTANCEID}','${process.contractNo}','${process.periodNum}');">任务下发</a></td>
							</c:if>
							<c:if test="${process.TASKDEFINITIONKEY ne 'utask_dqxf'}">
								<td>
									<a href="javascript:void(0)" onclick="details('${process.applyNo}');">详情</a>&nbsp;&nbsp;
									<a href="javascript:void(0)" onclick="handle('${process.PROCESSDEFINITIONID }','${process.EXECUTIONID }','${process.TASKDEFINITIONKEY }','${process.applyNo }','${process.NAME}','${process.ID }','${process.PROCESSINSTANCEID}','${process.contractNo}','${process.periodNum}')">结论</a>
