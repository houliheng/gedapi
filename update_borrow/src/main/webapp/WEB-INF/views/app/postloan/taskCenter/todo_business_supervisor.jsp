...
<script type="text/javascript">
	...
	function showInfo(procInstId,applyNo,title){
		var url = "${ctx}/credit/taskCenter/infoView?status=0&procInstId=" + procInstId+"&applyNo="+applyNo+"&title="+encodeX("详情查看---"+title);
    	var windowWidth = window.parent.window.document.body.offsetWidth -50;
		var windowHeight = window.parent.window.document.body.offsetHeight - 50;
		window.showModalDialog(url, null, "dialogWidth:" + windowWidth + "px" + ";dialogHeight:" + windowHeight + "px" + ";status:no;help:no;resizable:yes;");
	}
...
<body>
	<div class="wrapper">
		<div class="searchInfo">
			...
			<div class="searchCon">
				<form:form id="searchForm" modelAttribute="taskVO" action="${ctx}/credit/taskCenter/todoSupervisor" method="post">
						...
						<table class="searchTable">
							...
							<tr>
								...
								<td class="ft_content" style="width: 240px;">
									<input id=applyStartTime name="applyStartTime" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate" value="<fmt:formatDate value="${taskVO.applyStartTime }" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});" />
									<label>&nbsp;--&nbsp;</label>
									<input id="applyEndTime" name="applyEndTime" type="text" readonly="readonly" maxlength="20" class="input-mini Wdate" value="<fmt:formatDate value="${taskVO.applyEndTime }" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});" />
								</td>
							</tr>
						</table>
			...
			<div id="tableDataId" style="max-height:400px;overflow:auto;">
				<table cellpadding="0" cellspacing="0" border="0" width="100%">
					...
					<c:forEach items="${page.list}" var="process" varStatus="index">
						...
						<td class="title">
							<a href="javascript:void(0)" id="c_${index.count}_alink" onclick="showInfo('${process.PROC_INS_ID}','${process.CONTRACT_NO}','${process.CUST_NAME}')">${process.NAME}</a>
						</td>
