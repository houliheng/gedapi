...
<body onload="propertyShow();">
	<form:form id="carForm" modelAttribute="mortgageCarInfo" action="${ctx}/postloan/mortgageCarInfo/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
				...
				<table class="fromTable">
					...
					<tr>
						...
						<td class="ft_content" colspan="4">
							<form:select path="operateProvince" id="operateProvince" class="input-small required" onchange="loadCity('reg')">
								<option value="">请选择</option>
								<form:options items="${regProvinceMap}" htmlEscape="false" />
							</form:select>
							&nbsp;省
							<font color="red">*</font>
							<form:select path="operateCity" id="operateCity" class="input-small required" onchange="loadDistinct()">
								<option value="">请选择</option>
								<form:options items="${regCityMap}" htmlEscape="false" />
							</form:select>
							&nbsp;市
							<font color="red">*</font>
							<form:select path="operateDistinct" id="operateDistinct" class="input-small required">
								<option value="">请选择</option>
	...
	<c:if test="${not empty closeWindow}">
		<script type="text/javascript">
			alertx('${message}', function() {
				parent.$.loadDiv("mortgageCarEvaluate", "${ctx}/postloan/mortgageCarInfo/list", {applyNo:'${mortgageCarInfo.applyNo}'}, "post");
				closeJBox();
			});
		</script>
	</c:if>
