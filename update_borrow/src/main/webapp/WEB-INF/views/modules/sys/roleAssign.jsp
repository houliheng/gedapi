...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>归属机构</th><th>归属部门</th><th>登录名</th><th>姓名</th><th>电话</th><th>手机</th><shiro:hasPermission name="sys:user:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>		
		<c:forEach items="${userList}" var="user">
			<tr>
				<td>${user.company.name}</td>
				<td>${user.office.name}</td>
				<%-- <td><a href="${ctx}/sys/user/form?id=${user.id}">${user.loginName}</td>  --%>		
				<td><a id="userm" onclick="yonghuxingxinxi('${user.id}')" style="cursor:pointer;">${user.loginName}</a></td> 
				<td>${user.name}</td>
				<td>${user.phone}</td>
				<td>${user.mobile}</td>
    ...
    @date-author:20150717-laiwenchao:点击登录名后，利用js代码对用户信息以弹出窗口的方式进行呈现，该窗口无其他按钮，只有右上角的关闭按钮来关闭此窗口。
    -->
	<script type="text/javascript">
		function yonghuxingxinxi(userid){
			/**
			 * @reqno:H1507080068
			 * @date-designer:20150721-zhangxingyu   
			 * @date-author:20150721-zhangxingyu:去掉iframe:  url后面无用的isCheck参数
			 */
		  top.$.jBox.open("iframe:${ctx}/sys/user/table?id="+userid, "用户信息",810,$(top.document).height()-240,{
					buttons:{}, bottomText:"",submit:function(f){	
					}, loaded:function(h){
						$(".jbox-content", top.document).css("overflow-y","hidden");
					}
				});
