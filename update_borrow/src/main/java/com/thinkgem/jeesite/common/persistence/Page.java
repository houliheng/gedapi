...
public class Page<T> {
	 ...
	 * @date-author:20150724-chenshaojia:优化Mybatis分页参数传入方式,增加起始行、末行两个参数，用于分页参数的赋值
	 */
	 ...
	 * @reqno:H1507220015
	 * @date-designer:20150724-chenshaojia
	 * @date-author:20150724-chenshaojia:优化Mybatis分页插件的查询总数的sql语句，增加变量控制是否启用原有的分页count sql，默认为启用
	 */
	...
	public void initialize(){
		...
		this.first = 1;
		if(isDefaultCount()){
			this.last = (int) (count / (this.pageSize < 1 ? 20 : this.pageSize) + pageNo - 1);
		}else {
		...
		if(isDefaultCount()){
			if (this.pageSize > (int)this.count ) {
				this.last = this.pageNo;
				this.lastPage = true;
			}
			if (!lastPage) {
	...
	public String toString() {
		...
		sb.append("<input maxlength=\"4\" type=\"text\" value=\""+pageNo+"\" onkeypress=\"var e=event||window.event;var c=e.keyCode||e.which;if(c==13)");
		sb.append(funcName+"(this.value,"+pageSize+",'"+funcParam+"');\" onclick=\"this.select();\"/>页， 每页");
		
		sb.append("<input maxlength=\"3\" type=\"text\" value=\""+pageSize+"\" onkeypress=\"var e=event||window.event;var c=e.keyCode||e.which;if(c==13)");
	...
	public static void main(String[] args) {
		Page<String> p = new Page<String>(3, 30);
	...
	public void setPageSize(int pageSize) {
		this.pageSize = pageSize <= 0 ? Integer.valueOf(Global.getConfig("page.pageSize")) : pageSize;// > 500 ? 500 : pageSize;
	}
	...
	public Page<T> setList(List<T> list) {
		...
		/**
		 * @reqno: H1507070002
		 * @date-designer:2015年7月7日-zhunan
		 * @date-author:2015年7月7日-zhunan: 不需要在此处进行页号的初始化，已经在SQLHelper.generatePageSql中进行了
		 */
//		initialize();
		/*@end*/
		return this;
