package com.resoft.postloan.taskCenter.web;
...
import com.resoft.postloan.taskCenter.entity.TaskVO;
import com.resoft.postloan.taskCenter.service.TaskCenterService;
 ...
 * 任务中心 Controller
 */
@Controller("plTaskCenterController")
@RequestMapping(value = "${adminPath}/postloan/taskCenter")
public class TaskCenterController extends BaseController {
	...
	public String todoList(HttpServletRequest request, HttpServletResponse response, Model model, TaskVO taskVO) {
		...
		paramMap.put("procDefIdPre", Constants.WORKFLOW_CONTRACT_EXTEND);
		Page<MyMap> page = taskCenterService.findContractExtendToDoList(new Page<MyMap>(request, response), paramMap);
		...
		return "app/postloan/taskCenter/todo_business_list_exetend";
	...
	private MyMap setParamMap(TaskVO taskVO) {
		...
		if (taskVO != null) {
			...
			if (StringUtils.isNotBlank(taskVO.getContractNo())) {
				paramMap.put("contractNo", taskVO.getContractNo());
			}
	...
	public String toDoneList(HttpServletRequest request, HttpServletResponse response, Model model, TaskVO taskVO) {
		MyMap paramMap = setParamMap(taskVO);
		paramMap.put("procDefIdPre", Constants.WORKFLOW_CONTRACT_EXTEND);
		Page<MyMap> page = taskCenterService.findContractExtendDoneList(new Page<MyMap>(request, response), paramMap);
		model.addAttribute("page", page);
		if (taskVO.getOffice() != null) {// LevelnumString为空时，回显错误
			if (!"4".equals(taskVO.getOffice().getLevelnumString()) && !"3".equals(taskVO.getOffice().getLevelnumString()) && !"2".equals(taskVO.getOffice().getLevelnumString())) {// 登记门店
				taskVO.setOffice(new CustomOffice());
		...
		return "app/postloan/taskCenter/todone_business_list_exetend";
	}
	...
	public String toClaimList(TaskVO taskVO, Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		model.addAttribute("page", page);
		return "app/postloan/taskCenter/toclaim_business_list";
	}
	...
	public String todoListDynamic(HttpServletRequest request, HttpServletResponse response, Model model, TaskVO taskVO, @PathVariable(value = "taskDefKey") String taskDefKey, String returnVal) {
		...
		paramMap.put("procDefIdPre", Constants.WORKFLOW_COLLATERAL_DISPOSAL);
		if (Constants.ADMIN_USER_ID.equalsIgnoreCase(UserUtils.getUser().getId())) {
			Page<MyMap> page = taskCenterService.findTaskToDoListAdmin(new Page<MyMap>(request, response), paramMap);
			model.addAttribute("page", page);
		} else {
			Page<MyMap> page = taskCenterService.findTaskToDoList(new Page<MyMap>(request, response), paramMap);
			model.addAttribute("page", page);
		}
		model.addAttribute("taskDefKey", taskDefKey);
		return "app/postloan/taskCenter/todo_business_list";
	}
	...
	public String toDoneListDynamic(HttpServletRequest request, HttpServletResponse response, Model model, TaskVO taskVO, @PathVariable(value = "taskDefKey") String taskDefKey) {
		...
		paramMap.put("taskDefKey", taskDefKey);
		paramMap.put("procDefIdPre", Constants.WORKFLOW_COLLATERAL_DISPOSAL);
		Page<MyMap> page = taskCenterService.findTaskToDoneList(new Page<MyMap>(request, response),paramMap);
		model.addAttribute("page", page);
		model.addAttribute("taskDefKey", taskDefKey);
		return "app/postloan/taskCenter/todone_business_list";
	}
	...
	public String todoSupervisor(HttpServletRequest request, HttpServletResponse response, Model model, TaskVO taskVO) {
		...
		return "app/postloan/taskCenter/todo_business_supervisor";
	}
	...
	public String todoneSupervisor(HttpServletRequest request, HttpServletResponse response, Model model, TaskVO taskVO) {
		...
		model.addAttribute("page", page);
		return "app/postloan/taskCenter/todone_business_supervisor";
	}
	...
	public String processTrack(HttpServletRequest request, HttpServletResponse response, Model model, String procInstId) {
		...
		return "app/postloan/taskCenter/processTrack";
	}
	...
	public @ResponseBody
	Map<String, String> getTaskAddr(@PathVariable("procDefId") String procDefId, @PathVariable("taskDefKey") String taskDefKey, HttpServletResponse response, String taskId) throws Exception {
	...
	public String toBatchSubmit(String procInsIds, String taskIds, Model model, TaskVO taskVO) throws Exception {
		...
		model.addAttribute("taskVO", taskVO);
		return "app/postloan/taskCenter/todo_batch_business_edit";
	}
	...
	public String toInfoView(ActTaskParam actTaskParam, Model model, String readOnly) {
		...
		return "app/postloan/taskCenter/process_info_index";
	}
