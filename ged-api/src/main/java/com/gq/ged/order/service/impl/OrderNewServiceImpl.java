...
public class OrderNewServiceImpl implements OrderNewService {
  ...
  /*
   * @Value("${gq.ged.repayment.url}") String repaymentUrl;
   */
  /*
   * @Value("${gq.ged.orderCreate.url}") String orderCreateUrl;
   */
  @Value("${gq.borrow.url}")
  String borrowUrl;
  /*
   * @Value("${gq.ged.feedBack.url}") String feedBackUrl;
   */
  /*
   * @Value("${gq.ged.withDrawCash.url}") String withDrawUrl;
   */
  /*
   * @Value("${gq.ged.accountBalance.url}") String accountBalanceUrl;
   */

  @Value("${gq.ged.call.capital.url}")
  String capitalUrl;
  /*
   * @Value("${gq.ged.rechargeCallBack.url}") String rechargeCallBackUrl;
   */
  /*
   * @Value("${gq.ged.compensation.url}") String compensationUrl;
   */
  /*
   * @Value("${gq.ged.repaymentWithholding.url}") String repaymentWithholdingUrl;
   */
  ...
  @Override
  public RepayPlanTotleResForm selectOrderStatus(User user) throws Exception {
    ...
    List<GedOrder> list = selectGedOrder(user.getId());
    Integer accStatus = selectAccStatus(user);
    logger.info("开户状态==" + accStatus);
    Account account = accountService.getAccountByUserId(userId);
    AccountCompany accountCompany = accountService.queryCompanyAccountByUserId(userId);
    Integer flag = 0;
    if (account != null) {
      resForm.setCompanyName(account.getCompanyName());// 开户公司名称
      flag = account.getStatus();
    }

    if (accountCompany != null) {
      //0 企业未开户/个人未开户  1企业/个人开户中 2企业/个人 已开户 3企业开审核退回/企业开审核拒绝
      flag = accountCompany.getStatus();
    }

    resForm.setAccStatus(flag);// 开户状态
      ...
      if (StringUtils.isNotBlank(feStatus)) {
        switch (feStatus) {
          ...
          case "S40":// 提现处理中 放款成功
            ...
            // 账号余额
            resForm.setAccountBalance(accountService.queryAccountBalance(userId));
            break;
          case "S50":// 还款中
            resForm.setOrderNo(gedOrder.getOrderCode());
            resForm.setStatus(feStatus);
  ...
  public Integer selectAccStatus(User user) {
    return accountService.selectAccStatus(user);
  }
  ...
  public RepayPlanTotleResForm halfPlan(List<DeductResult> list, RepayPlanTotleResForm resForm)
    ...
    if (list != null && list.size() != 0) {
      for (DeductResult plan : list) {
        ...
        if (plan.getRepayPeriodStatus().equals("0300")) {// 逾期
      ...
      if (flag == true) {// 有逾期
        int repayDay = 0;
        for (DeductResult plan : list) {
          if (plan.getRepayPeriodStatus().equals("0300")) {
        ...
        for (DeductResult plan : list) {
          // 查询是否有下一期的 金额和期限
          if (plan.getRepayPeriodStatus().equals("0200") && simpleDateFormat
  ...
  public AccountWheResForm selectWheAccount(User userInfo) {
    ...
    // 查询企业是否开户
    AccountCompany accountCompany = accountService.queryCompanyAccountByUserId(userInfo.getId());
    if (accountCompany == null) {
      resForm.setFlag(0);
      return resForm;
    }
  ...
  public WithDrawResForm withDraw(User userInfo, String orderNo) throws Exception {
    ...
    BigDecimal accountBalan = accountService.queryAccountBalance(userInfo.getId());
  ...
  public String getContractUrlByContractNo(String contractNo) throws Exception {
    ...
    if (res == null || !res.getString("code").equals("0000")) {
  ...
  public WithDrawCapitalResForm withDr(long userId, String orderNo) throws Exception {
    ...
    BigDecimal accountBalan = accountService.queryAccountBalance(userId);
    ...
    if (!json.getString("resp_code").equals("0000")) {
      throw new BusinessException(Errors.PUSH_ERROR);
    }
  ...
  public Map<String, Object> callBackwithDraw(CallBackWithDrawReqForm reqForm) throws Exception {
    ...
    if (reqForm.getResp_code().equals("0000")) {// 提现成功
      ...
      if (gedOrder != null) {
        ...
        // if (!loanJson.getString("code").equals("0000")) {
        // throw new BusinessException(Errors.PUSH_ERROR);
        // }
  ...
  public WithDrawViewResForm beforeWithDr(String orderNo, Long userId) throws Exception {
    ...
    AccountCompany accountCompany = accountService.getAccountCompanyByUserId(userId);

    WithDrawViewResForm res = new WithDrawViewResForm();

    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");

    String accountAmt = accountService.queryAccountBalance(userId).setScale(2, BigDecimal.ROUND_HALF_DOWN) + "";
    res.setAccountAmt(accountAmt);
    res.setCanWithdrawAmt(order.getCreditAmount().subtract(order.getWithdrawAmount() == null ? new BigDecimal(0) : order.getWithdrawAmount()).setScale(2, BigDecimal.ROUND_HALF_DOWN) + "" );

   if(account != null){
     res.setBankNum(account.getCompanyAccount() == null ? ""
             : account.getCompanyAccount().substring(account.getCompanyAccount().length() - 4));
     res.setCorporationName(account.getCorporationName());//个人户姓名
     res.setBankName(account.getCompanyBankOfDeposit());
   }else if(accountCompany != null){
     //银行卡编号
     res.setBankNum(accountCompany.getCompanyAccount() == null ? ""
             : accountCompany.getCompanyAccount().substring(accountCompany.getCompanyAccount().length() - 4));
     res.setCorporationName(accountCompany.getLegalName());//企业户法人姓名
     res.setBankName(accountCompany.getCompanyBankOfDeposit());
   }
  ...
  public RechargeReForm userRecharge(Long userId, String bankId) throws Exception {
    ...
    if (res != null && res.getString("resp_code").equals("0000")) {
      ...
      saverRechargeRecordItem(order, r.getUserName(), r.getOrderNo(), amt, bankId);
      return r;
    }
    throw new BusinessException(Errors.SYSTEM_ERROR,
  ...
  public void saverRechargeRecordItem(GedOrder order, String username, String order_no, String amt,
    ...
    rechargeRecord.setRespCode("0000");
  ...
  @Override
  public void userRechargeCallBackForRepayment(String orderNo, String amt,
                                               PayCallBackReqForm payCallBackReqForm) throws Exception {
    logger.info("↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓还款充值成功后回调get开始↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓第三方流水号=" + orderNo);
    if (!payCallBackReqForm.getResp_code().equals("0000")) {
      throw new BusinessException(Errors.SYSTEM_ERROR, "还款充值前获取流水号失败 原因："
              + payCallBackReqForm.getResp_code() + "-" + payCallBackReqForm.getResp_desc());
    }
    GedRepaymentRecord record = this.getGedRepaymentRecordByOrderNo(orderNo);
    ...
    rechargeCallBackForm.setSeq_no(order.getOrderCode() + dateFormat.format(new Date()));// 交易流水号

    rechargeCallBackForm.setCust_no(userInfo.getGetCustId());// 客户编号
    rechargeCallBackForm.setOrder_no(orderNo);// 第三方交易流水号
    // rechargeCallBackForm.setTradeDate(rechargeRecord.getTradeDate().getTime() + "");// 第三方交易日期
    ...
    logger.info("******************************还款充值完成后调get入账的参数******************************");
    logger.info(JSONObject.toJSONString(rechargeCallBackForm));
    JSONObject res = this.rechargeCallBack(rechargeCallBackForm);
    logger.info("******************************还款充值成功回调操get作结果为：");
    logger.info(res.toJSONString());
    Map<String, String> data = new HashedMap();
    ...
    recorde.setAccountBalance(accountService.queryAccountBalance(order.getUserId())); // 账户余额
    this.insertOrUpdate(recorde);
...
//    paraForm.setSeq_no(order.getOrderCode() + dateFormat.format(new Date()));
    ...
    JSONObject res = this.compensation(paraForm);
    logger.info(
            "*********************************************缴费操作后返回的结果为*********************************************");
    ...
    if (res == null || !res.getString("resp_code").equals("0000")) {
      ...
      rechargeRecordService.updateById(record);
      // throw new BusinessException(Errors.ORDER_NOT_EXISTS_ERROR,
      // "缴费失败orderNo=" + order.getOrderCode());
      return resForm;
    }
  ...
  public GeneratePayParas generatePayParas(String seqNum, String loginName, String orderNo,
    ...
    payParas.setMchnt_txn_ssn(order.getOrderCode() + df.format(new Date()));
  ...
  public Integer saveResParasAfterPay(PayCallBackReqForm res) throws Exception {
    ...
    logger.info(JSONObject.toJSONString(res));
    // 第三方支付成功
    GedOrder order = this.selectOrderByOrderNo(
            res.getMchnt_txn_ssn().substring(0, res.getMchnt_txn_ssn().length() - 6));
    if (order == null)
      throw new BusinessException(Errors.ORDER_NOT_EXISTS_ERROR,
    ...
    if (res != null && res.getResp_code() != null && res.getResp_code().trim().equals("0000")) {
  ...
  public OrderFeesDetailReqForm payFeeInfos(Long userId) throws Exception {
    ...
    BigDecimal result = accountService.queryAccountBalance(userId);
  ...
  public RepayPlanResForm repaymentPlan(Long userId) throws Exception {
    ...
    if (list != null && list.size() > 0) {
      ...
      if (list.get(list.size() - 1).getRepayPeriodStatus().equals("0200")
              || list.get(list.size() - 1).getRepayPeriodStatus().equals("0300")) {
        res.setIsPay("1");
      }
    }
    ...
    if (thisPerisNum != -1) {
      // 判断当期 是否到达还款日期
      if (thisPerisNum > 0 && !list.get(thisPerisNum - 1).getRepayPeriodStatus().equals("0200")
  ...
  private void repayCopyItem(DeductResult deduct, RepaymentItem repaymentItem) {
    ...
    //add 2018/7/10
    repaymentItem.setServiceFee(deduct.getServiceFee()); // 服务费
    repaymentItem.setManagementFee(deduct.getManagementFee());// 账户管理费
    repaymentItem.setCapitalAmount(deduct.getCapitalAmount());//本金
    repaymentItem.setInterestAmount(deduct.getInterestAmount());//利息
  ...
  public Integer updateServiceFlagByOrderNo(RpdateServiceFlagByOrderNoReqForm req) {
    ...
    //0债股结合1非债股结合（债股结合产品需要用接口系统传过来的服务费金额 更新冠e贷的订单表）
    if(StringUtils.isNotBlank(req.getType()) && "0".equals(req.getType())){
      updateOrder.setServiceFee(new BigDecimal(req.getFactServiceFee()));//服务费
      updateOrder.setServiceFeeFlag(1);//服务费标识
    }
    /*if(StringUtils.isNotBlank(req.getFactServiceFee()) && !req.getFactServiceFee().equals("0.00")  ) {
      updateOrder.setServiceFee(new BigDecimal(req.getFactServiceFee()));//服务费
      updateOrder.setServiceFeeFlag(1);//服务费标识
    }*/
  ...
  public PushToBorrowRepaymentResForm pushToBorrowRepayment(String orderNo) throws Exception {
    logger
            .info("---------------------------调用借款端进行还款beigin----------------------------------------");
    GedRepaymentRecord record = getGedRepaymentRecordByOrderNo(orderNo);
    if (record == null) {
    ...
    // JSONObject json = repay(reqForm);
    repay(reqForm);
    logger.info("---------------------------调用借款端进行还款end---------------------------------------");
    // logger.info(json.toJSONString());
    /*
     * if (!json.getString("resp_code").equals("0000")) { // GedRepaymentRecord recorde = new
     * GedRepaymentRecord(); // recorde.setStatus(3); //
     * recorde.setDeductApplyNo(reqForm.getDeductApplyNo()); // this.insertOrUpdate( recorde); throw
     * new BusinessException(Errors.PUSH_ERROR, "还款推送借款处理失败"); }
     */
    /*
     * RBucket rBucket =redissonClient.getBucket(Constant.CONTRACT_NO + ":" +
     * order.getContractCode()); rBucket.set(1);
     */
    /*
  ...
  private void insertOrUpdate(GedRepaymentRecord recorde) {
  ...
  public Integer getRepayStatus(Long userId) {
     ...
     * return 1; }
     */
    return order.getRepaymentLockFlag() == null ? 0 : order.getRepaymentLockFlag();
  }
  ...
  public void updateRepayStatusAfterRecharge(GedOrder gedOrder) {
    if (gedOrder != null) {
      GedOrder updater = new GedOrder();
      updater.setId(gedOrder.getId());
      updater.setRepaymentLockFlag(0);
      logger.info(
              "================================还款充值成功后锁住，将订单状态设置为还款进行中==================================orderNo" + gedOrder.getOrderCode());
      this.gedOrderMapper.updateByPrimaryKeySelective(updater);
    }
  }
