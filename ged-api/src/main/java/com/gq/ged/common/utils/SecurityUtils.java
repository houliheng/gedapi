...
import com.gq.ged.account.service.impl.RsaKeyServiceImpl;
...
//				privateKeyPath = ConfigReader.getConfig("privateKeyPath");
				privateKey = getPrivateKey(RsaKeyServiceImpl.prik);
			}
		} catch (Exception e) {
...
//				publicKeyPath = ConfigReader.getConfig("publicKeyPath");
				publicKey = getPublicKey(RsaKeyServiceImpl.pubk);
			}
		} catch (Exception e) {
			System.out.println("SecurityUtils初始化失败" + e.getMessage());
			e.printStackTrace();
	...
	private static PrivateKey getPrivateKey(String base64edKey) {
//		String base64edKey = readFile(filePath);
	...
	private static PublicKey getPublicKey(String base64edKey){
		KeyFactory kf;
